...
public class RestAPITest {
	...
	@Test
	public void testSearchByRamRange() {
		final String url = "http://localhost:8080/cormacLaptops/rest/laptops/ram/5/8";
		final RestResponse response = RestApiHelper.performGetRequest(url, null);
		assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		final Laptop[] laptops = new Gson().fromJson(response.getResponseBody(), Laptop[].class);
		assertEquals(2, laptops.length);
		assertEquals("Lenovo", laptops[0].getMake());
		assertEquals("Q50", laptops[0].getModel());
	}
	...
	public void testDeleteLaptop() {
		...
		Laptop[] LaptopList = new Gson().fromJson(response.getResponseBody(), Laptop[].class);
		assertEquals(3, LaptopList.length);
		...
		response = RestApiHelper.performGetRequest(url, null);
		LaptopList = new Gson().fromJson(response.getResponseBody(), Laptop[].class);
		assertEquals(2, LaptopList.length);
	}
